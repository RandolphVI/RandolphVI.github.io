...
description: "本文主要介绍了一些本人在 Kaggle 上首战的经验，包括数据的 EDA、FE 特征工程、所选择的模型、模型融合以及其他。"
draft: false
---
...
- 上下文特征：用户对音乐做出的某项行为，该行为的 metadata，例如 `registration_init_time` 注册的时间、`source_type` 使用的设备等

有些特征是在资料 EDA 阶段就可以拿到，有些特征则需要额外的步骤（例如透过外部的 API 或者其他模型）才能取得。
...
- 音乐特征
  ...
  - `is_featured`：判断 `artist` 中是否存在 `feat.` 信息（<strong>`feat.`</strong> 是 **featuring** 的缩写，如果直译的话，指「以……为特色、亮点」。<strong>`feat.`</strong> 在歌曲里面是指专辑表演者与另外（一个或者多个）的艺人/组合的合作，也就是请人在歌曲中客串。）
  - `smaller_song`：判断 `song_length` 是否小于 `avg_song_length` （`avg_song_length` 是 **train & test** 所出现的所有 `song_length` 的平均长度） 
  - `artist_composer`：判断 `artist` 与 `composer` 中是否出现同样的艺人，如果有则记为 1，否则记为 0（间接反映艺人的有才程度）
  - `artist_composer_lyricist`：判断 `artist` 、 `composer` 以及 `lyricist` 中是否出现同样的艺人，如果有则记为 1，否则记为 0（间接反映艺人的有才程度）
  ...
  - `count_genre_like`： 该曲风 `genre_ids` 在 **train** 中 <strong>`target=1`</strong>（即被喜欢）的次数
...
- 针对 numerical 特征的缺失值，可以用以下方式取代：
  ...
  - Mean，平均数（例如用户年龄信息 `bd` 存在许多异常值（存在负数、零甚至超过一百），对于那些异常值可以用 `bd` 的平均值来代替）
  - Median，中位数，跟平均数相比，不会被异常值干扰
- 针对 categorical 特征的缺失值，可以用以下方式取代：
  - Mode，众数，最常见的值
  - 改成 `Others` 之类的值

假设你要填补 `bd` 这个特征，然后你有其他例如 `gender` 这样的特征，你可以分别计算男性和女性的 `bd` 的平均数、中位数和众数来填补缺失值；更复杂一点的方式是，你可以把没有缺失值的数据挑出来，用它们来训练一个 regression 或 classification 模型，用这个模型来预测缺失值。
...
### 2.3 Duplicate Entries Removal 重复值去除
...
- <strong>对 Vector Space Model 来说，需要 normalization</strong>；
- **对 tree-based 的算法，基本上都不需要标准化或归一化，它们对 scale 不敏感。**

### 2.5 Feature Transformation 特征变换

针对 continuous 特征，可以进行 Feature Transformation 特征变换：
...
因为 $x$ 越大，$log(x)$ 增长的速度就越慢，所以<strong>取 log 的意义是可以 compress 大数和 expand 小数</strong>，换句话说就是压缩 "long tail" 和展开 "head"。假设 x 原本的范围是 $[100, 1000]$，$log(x, 10)$ 之后的范围就变成 $[2, 3]$ 了。也常常使用 $log(1 + x)$ 或 $log(\frac{x}{1-x})$。
...
也称为 <strong>bucketization</strong>。以 `age` 这样的特征为例，你可以把所有年龄拆分成 $n$ 段，0-20 岁、20-40 岁、40-60 岁等或是 0-18 岁、18-40 岁、40-70 岁等（等距或等量），然后把个别的年龄对应到某一段，假设 26 岁是对应到第二个 bucket，那新特征的值就是 2。这种方式是人为地指定每个 bucket 的边界值，还有另外一种拆分法是根据数据的分布来拆，称为 quantization 或 quantile binning，你只需要指定 bucket 的数量即可。

同样的概念应用到其他地方，可以把 datetime 特征拆分成上午、中午、下午和晚上；如果是 categorical 特征，可以把出现次数小于某个 threshold 的值改成 `Other` 之类的。或者是你有一个 occupation 特征，如果你其实不需要非常准确的职业资讯的话，可以把 "Web Developer"、"iOS Developer" 或 "DBA" 这些个别的资料都改成 "Software Engineer"。

**Binarization 和 Binning 都是对 continuous 特征离散化，增强模型的非线性泛化能力。**

#### **2.5.5 Integer Encoding**

也称为 <strong>label encoding</strong>。把每个 category 对应到数字，一种做法是随机对应到 0, 1, 2, 3, 4 等数字；另外一种做法是依照该值出现的频率大小的顺序来给值，例如最常出现的值给 0，依序给 1, 2, 3 等等。如果是针对一些在某种程度上有次序的 categorical 特征（称为 ordinal），例如「钻石会员」「白金会员」「黄金会员」「普通会员」，直接 mapping 成数字可能没什么问题，但是如果是类似 `color` 或 `city` 这样的没有明显大小的特征的话，还是用 one-hot encoding 比较合适。不过如果用的是 tree-based 的算法就无所谓了。
...
One-hot Encoding 的缺点是容易造成特征的维度大幅增加和没办法处理之前没见过的值。
...
除了可以用在 text 特征之外，如果你有 comma-separated 的 categorical 特征也可以使用这个方法。例如电影类型 `genre`，里头的值为 `Action, Sci-Fi, Drama`，就可以先用 `RegexTokenizer` 转成 `Array("action", "sci-fi", "drama")`，再用 `CountVectorizer` 转成 vector。
...
Category Embedding 把高基数（high-cardinality）的类别特征映射成一个低维的稠密向量（dense embedding），这个向量在模型训练时和其它参数一起学出来——和 NLP 里的 word embedding、推荐里的 entity embedding 是同一个思路。相比 one-hot 把每个类别变成稀疏的高维向量，embedding 只用几十维就能表示，既压缩了维度，又能让「语义相近」的类别在向量空间里靠得更近（例如两个曲风相似的歌手）。实务上常用一个 `Embedding` 层或浅层网络学到这些向量，再当作下游模型的输入特征。缺点是需要足够的数据才能学出有意义的表示，对罕见类别效果有限。
...
先计算好每一种 category 的数量，然后把小于某个 threshold 的 category 都改成 `Others` 之类的值。或是使用 clustering 算法来达到同样的目的。你也可以直接建立一个新的 binary feature 叫做 `rare`，要来标示那些相对少见的类别。
...
如果采用第一种方式，一旦你把这个 transformer 拿到 production 去用时，无可避免地还是会遇到 unseen values。不过通常线上的 feature engineering 会有别的做法，例如事先把 user 或 item 的各项特征都算好（定期更新或是 data 产生的时候触发），然后以 id 为 key 存进 Redis 之类的 NoSQL 里，model 要用的时候直接用 user id / item id 拿到处理好的 feature vector。
...
对于文字类型的栏位（歌名、歌手名、专辑名、用户搜索词等），可以抽取出多种特征：最基础的是文字长度、词数、是否包含某些关键字这类统计量；进一步可以用 bag-of-words、TF-IDF、n-gram 把文字转成向量；也可以用预训练 word embedding 取平均得到句向量，或直接用 `word2vec`/`fastText` 训练领域内的词向量。对推荐场景，一个很强的特征是计算「用户历史文本」与「候选物品文本」之间的相似度（如 TF-IDF cosine 或 embedding 相似度）。

#### **2.6.3 Spatial Features 地理特征**

对于带地理位置的资料（经纬度、城市、注册地、IP 归属地等），可以构造：两点之间的距离（如 haversine 距离）、所属的行政区/商圈、把经纬度做 binning 或用 KMeans 聚类得到「区域」类别、到城市中心的距离等。对音乐推荐这类场景，地理特征更多体现在「用户所在地区」与内容流行度的关联——某些歌曲在特定地区更受欢迎，可以据此构造「该歌曲在用户所在地区的播放占比」这类特征。

#### **2.6.4 Cyclical Features**

很多时间类特征本质是「循环」的——小时（0 点和 23 点其实相邻）、星期、月份、角度等。若直接当成数值或类别，模型会丢失「首尾相接」的信息。常见做法是用正余弦把它编码到一个圆上：对周期为 $T$ 的特征 $x$，构造 $\sin(2\pi x / T)$ 和 $\cos(2\pi x / T)$ 两个新栏位，这样 23 点和 0 点在向量空间里就会很接近。对小时（$T=24$）、星期（$T=7$）、月份（$T=12$）都可以这样处理。
...
<strong>特征组合主要是针对 categorical 特征，特征交互则是适用于连续值特征</strong>。但是两者的概念是差不多的，就是把两个以上的特征透过某种方式结合在一起，变成新的特征。通常用来解决一般的线性模型没办法学到非线性特征的问题。
...
这个模型就是 **Field-aware Factorization Machines（FFM，场感知因子分解机）**——FM 的升级版，让每个特征针对不同的「场（field）」各学一组隐向量，在 CTR 预估这类高维稀疏数据上表现很好（可参见笔记 [About Field-aware Factorization Machines](/tech/about-field-aware-factorization-machines/)）。要使用它，需要先下载 [LIBFFM](http://www.csie.ntu.edu.tw/~r01922136/libffm)。

标题里的「+ GBDT」指的是一种常见的组合打法：先用 GBDT 跑一遍，把每条样本落到各棵树的叶子节点编号（即前文的 `gbdt_model_index` 特征）当作新的离散特征，再喂给 FFM——用 GBDT 自动完成特征组合、再由 FFM 学习高阶交叉，是当年 CTR 竞赛里很有效的一套方案。

### CatBoost

CatBoost 是 Yandex 开源的梯度提升（GBDT）库，最大特点是**原生、优雅地处理类别特征**——内部用「有序目标编码」（ordered target statistics）自动把类别变量转成数值，省去手动做 one-hot / target encoding 的麻烦，也降低了目标泄漏（target leakage）的风险；它还用「有序提升」（ordered boosting）缓解过拟合。默认参数下往往就有不错表现，特别适合类别特征多的场景（推荐、CTR 正是如此）。

### XGBoost

XGBoost（eXtreme Gradient Boosting）是竞赛里最经典的梯度提升实现，在 GBDT 基础上加入了正则项（控制叶子数与权重）、损失函数的二阶泰勒展开、列采样、缺失值自动处理等优化，兼顾精度与速度，长期是 Kaggle 结构化数据竞赛的主力。使用前需要先把类别特征编码成数值。

### LightGBM

LightGBM 是微软开源的梯度提升库，主打**大数据下的速度与内存效率**：采用基于直方图（histogram）的分裂、leaf-wise（按叶子）的生长策略（而非 XGBoost 默认的 level-wise），并通过 GOSS、EFB 等技术进一步加速。在大规模、高维稀疏数据上通常比 XGBoost 更快、精度相当，也支持类别特征。
