...
自增计数器。前面的九个字节是保证了一秒内不同机器不同进程生成`ObjectId`不冲突，这后面的三个字节<strong>`36236b`</strong>是一个自动增加的计数器，用来确保在同一秒内产生的`ObjectId`也不会发生冲突，允许$256^3=16777216$条记录的唯一性。
...
`MongoDB` 的`save()`和`insert()`函数都可以向`collection`里插入数据，但两者有两个区别：
...
2. <strong>`multi`</strong>：是否删除多个文档。默认是删除多条符合条件的文档。
...
1. <strong>`filter`</strong>：过滤器，用于定位需要查询的目标文档。
...
同样地，和 `insert()` 插入操作一样，除了 `find()` 查询操作之外，还有 `find_one()`、`find_and_modify()` 等等。
