...
举个例子， **`4e7020cb7cac81af7136236b`** 这个 24 位的字符串，虽然看起来很长，也很难理解，但实际上它是由一组十六进制的字符构成，每个字节两位的十六进制数字，总共用了12字节的存储空间。
...
可以使用python的内置函数`repr()`来获取可识别的字符串，其中十六进制的 **`0x00000014`**, **`0x00001009`** 分别表示 **`20`** 和 **`400`** 。
