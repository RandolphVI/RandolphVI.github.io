...
Tensorboard 提供了多个功能模块，用于可视化神经网络训练过程中的不同类型数据，主要包括以下几大模块：

- <strong>SCALARS</strong>：记录单一变量的值，使用 **`tf.summary.scalar()`** 收集构建。
- <strong>IMAGES</strong>：收集的图片数据，当我们使用的数据为图片时（选用）。
- <strong>AUDIO</strong>：收集的音频数据，当我们使用数据为音频时（选用）。
- <strong>GRAPHS</strong>[^2]：计算图，效果图类似流程图一样，我们可以看到数据的流向，使用<strong>`tf.name_scope()`</strong>收集构建。
...
[^2]: Tensorboard 的 GRAPHS 模块展示的是 TensorFlow 的静态计算图 （computational graph），与 PyTorch 所强调的 eager execution 的设计哲学有根本差异。静态图强迫用户提前定义计算流程，但也因此能进行更好的图优化和部署。
